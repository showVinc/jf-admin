...
<script>
  export default {
    ...
    methods: {
      ...
      editor(index){
        let self = this
        self.$router.push({path:'/news/add',query:{aid:self.tableData[index].aid}})
      },
      deleteRow(index){
        let self = this
        self.$fun.delete(`${process.env.API.API}/admin/news/arts`,{aid:self.tableData[index].aid},res=>{
          if(res.errcode=='0'){
    ...
    mounted(){
      let self = this
      self.$fun.get(`${process.env.API.API}/admin/news/arts`,{rows:10},res=>{
        for(let v of res.data){
          v.publish_time = self.$moment(v.publish_time * 1000).format('YYYY-MM-DD HH:mm')
        }
        self.tableData = res.data
